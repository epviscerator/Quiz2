...
static int compareEmployeeIncome(const void *targetPtr, PtrToConstEmployee tableValuePtr)
...
PtrToEmployee searchEmployeeByIncome(PtrToConstEmployee ptr, int size, double income)

{
    return searchEmployeeTable(ptr, size, &income, compareEmployeeIncome);
}
